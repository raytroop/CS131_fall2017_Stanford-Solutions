...
class PCA(object):
    ...
    def fit(self, X, method='svd'):
        ...
        if method not in ['svd', 'eigen']:
            raise ValueError("Method must be one of 'svd' or 'eigen'.")
        
        self.W_pca = self._svd(X_centered)[0] if 'svd' == method else self._eigen_decomp(X_centered)[0]
        # END YOUR CODE
    ...
    def _eigen_decomp(self, X):
        ...
        cov_mx = np.dot(X.T, X) / (N - 1)
        # np.linalg.eig: 60.7ms
        # np.linalg.eigh: 8.4ms
        e_vals, e_vecs = np.linalg.eig(cov_mx)
        idx = e_vals.argsort()[::-1]
        e_vals = np.real(e_vals[idx])
        e_vecs = np.real(e_vecs[:, idx])
    ...
    def reconstruct(self, X_proj):
        ...
        X = np.dot(X_proj, self.W_pca[:, :n_components].T) + self.mean
        # END YOUR CODE
...
class LDA(object):
    ...
    def fit(self, X, y):
        ...
        idx_sort = e_vals.argsort()[::-1]
        e_vals = e_vals[idx_sort]
        e_vecs = e_vecs[:, idx_sort]
        # END YOUR CODE
    ...
    def _between_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            mu_dif = mu_i - mu
            N_i = X_i.shape[0]
            scatter_between += np.dot(mu_dif[:, np.newaxis], mu_dif[np.newaxis, :]) * N_i
