...
def compute_distances(X1, X2):
    ...
    X1_expand = np.repeat(X1, N, 0)
    X2_expand = np.tile(X2, (M, 1))
    dists = np.linalg.norm(X1_expand - X2_expand, axis=1)
    dists = np.reshape(dists, (M, N))
    # END YOUR CODE
...
def predict_labels(dists, y_train, k=1):
    ...
    for i in range(num_test):
        ...
        # YOUR CODE HERE
        dist_i = dists[i]
        closest_y = y_train[np.argsort(dist_i)[:k]]
...
def split_folds(X_train, y_train, num_folds):
    ...
    # !!! Hint: np.array_split
    for n, i in enumerate(range(0, N, validation_size)):
        idx_val = idx_shuffle[np.logical_and(idx_range >= i, idx_range < i + validation_size)]
        idx_train = idx_shuffle[np.logical_or(idx_range < i, idx_range >= i + validation_size)]
        X_trains[n] = X_train[idx_train]
        y_trains[n] = y_train[idx_train]
        X_vals[n] = X_train[idx_val]
        y_vals[n] = y_train[idx_val]
